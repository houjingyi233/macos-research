...
LLVM_PROFILE_FILE=default.profraw ./ball-fuzz19

# Merge the profiling data
xcrun llvm-profdata merge -sparse default.profraw -o a.profdata

# Generate the textual report
xcrun llvm-cov report ./ball-fuzz19 -instr-profile=a.profdata

# Generate the HTML report and save it to a directory
xcrun llvm-cov show ./ball-fuzz19 -instr-profile=a.profdata -format=html -output-dir=./coverage_report

# Alternatively, if you prefer a single HTML file, uncomment the line below:
xcrun llvm-cov show ./ball-fuzz19 -instr-profile=a.profdata --format=html > coverage.html
